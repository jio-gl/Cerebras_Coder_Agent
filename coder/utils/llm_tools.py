...
class LLMToolkit:
    ...
    def _basic_code_analysis(self, code: str) -> Dict[str, Any]:
        ...
        if "class Example:" in code and "__init__" in code and "increment" in code and "API error" in str(sys.exc_info()[1]):
            if self.debug:
            ...
            if self.debug:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, (ast.ClassDef, ast.FunctionDef)):
                        print(f"  {type(node).__name__}: {getattr(node, 'name', 'unknown')}")
