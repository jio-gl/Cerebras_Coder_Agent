...
from coder.utils import VersionManager, CodeValidator, ValidationResult
...
class TestPathTraversalSecurity:
    ...
    def setup_method(self):
        ...
        (self.temp_dir.parent / "outside_file.txt").write_text("# Outside file - should not be accessible")
...
class TestFilePermissionSecurity:
    ...
    def test_backup_permission_preservation(self):
        ...
        original_items = ["coder", "tests", "setup.py", "README.md", "SPECS.md", "requirements.txt"]
        ...
        def patched_backup():
            ...
            backup_name = f"backup_v{current_version}_{int(time.time())}_{uuid.uuid4().hex[:8]}"
        ...
        for malicious_file in malicious_files:
            try:
                ...
                result = self.validator.run_command(
                    ["python", "-c", "import py_compile; py_compile.compile(r'{}')".format(str(file_path))],
                    "Test validation"
                )
    ...
    def test_timeout_security(self):
        ...
        if hasattr(self.validator, 'run_command'):
            ...
            result = quick_timeout_command(
                ["python", str(infinite_script)],
                "Infinite script test"
            )
        else:
    ...
    def test_secure_random_generation(self):
        ...
        with patch('coder.agent.time.time') as mock_time:
            ...
            with patch('coder.agent.OpenRouterClient'):
                ...
                assert backup_dir1.name != backup_dir2.name or "random" not in backup_dir1.name.lower()
