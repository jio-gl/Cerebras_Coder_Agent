...
"""
        ...
        monkeypatch.setattr(agent.client, "get_completion", lambda x: x["choices"][0]["message"]["content"])
    ...
    def test_fix_unbalanced_quotes(self, sample_file_with_unbalanced_quotes, monkeypatch):
...
"""
        ...
        for py_file in Path(sample_directory_with_errors).glob("**/*.py"):
            ...
            assert check_result.returncode == 0, f"File {py_file} still has syntax errors" 