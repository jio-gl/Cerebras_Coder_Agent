...
from coder.utils.version_manager import VersionInfo, VersionManager
...
class TestVersionInfo:
    ...
    def test_version_info_default_test_status(self):
        ...
        info = VersionInfo(
            version=1, path=Path("/tmp"), specs_content="specs", is_valid=True
        )
...
class TestVersionManagerIntegration:
    ...
    def test_concurrent_version_creation(self):
        ...
        with pytest.raises(
            ValueError, match=f"Version {version} directory already exists"
        ):
