...
"""
    ...
    def test_self_rewrite_flow(self, mock_client_class):
        ...
        with patch.object(
            self.agent.version_manager, "create_version_directory"
        ) as mock_create_dir, patch.object(
            self.agent, "_create_backup"
        ) as mock_create_backup, patch.object(
            self.agent, "_generate_improved_specs"
        ) as mock_gen_specs, patch.object(
            self.agent, "_generate_version_files", return_value=True
        ) as mock_gen_files, patch.object(
            self.agent, "_validate_new_version", return_value=True
        ) as mock_validate, patch.object(
            self.agent, "_generate_completion_summary"
    ...
    def test_self_rewrite_validation_failure(self, mock_client_class):
        ...
        with patch.object(
            self.agent, "_validate_new_version", return_value=False
        ), patch.object(
            self.agent, "_create_backup"
        ) as mock_create_backup, patch.object(
    ...
    def test_self_rewrite_file_generation_failure(self, mock_client_class):
        ...
        with patch.object(
            ...
            self.agent, "_generate_version_files", return_value=False
        ), patch(
            "subprocess.run"
        ) as mock_run:
...
class TestSelfRewritePerformance:
    ...
    def setup_method(self):
        ...
        self.mock_clean.side_effect = (
            lambda x: "# Generated file\n\ndef test():\n    pass"
        )
