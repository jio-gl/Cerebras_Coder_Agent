...
def test_agent_fixes_typo_in_node_repo(agent_node, example_node_repo):
    ...
    result = subprocess.run(
        [sys.executable, "-c", f"exec(open('{example_node_repo}/index.js').read())"],
        capture_output=True,
        text=True,
    )
...
def test_agent_runs_npm_install_and_start(agent_node, example_node_repo):
    ...
    install_proc = subprocess.run(
        ["npm", "install"], cwd=str(example_node_repo), capture_output=True, text=True
    )
    assert install_proc.returncode == 0
    # Run npm start
    start_proc = subprocess.run(
        ["npm", "start"], cwd=str(example_node_repo), capture_output=True, text=True
    )
    # Output should contain Hello, world!
    assert (
        "Hello, world!" in start_proc.stdout
        or "Hello, world!" in agent_node._read_file("index.js")
    )
...
def test_agent_generates_and_runs_command(agent_node, example_node_repo):
    ...
    command = "ls -1"
    proc = subprocess.run(
        command, cwd=str(example_node_repo), shell=True, capture_output=True, text=True
    )
