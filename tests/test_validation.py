...
""")
    ...
    def test_validate_imports_success(self):
        ...
        (self.temp_dir / "coder" / "utils" / "equivalence.py").write_text("class EquivalenceChecker: pass")
    ...
    def test_validate_imports_cleanup_on_exception(self):
        ...
        try:
            ...
            assert not script_path.exists(), "Script file should be cleaned up on exception"
...
""")
        ...
        # Test valid file
        result = self.validator.run_command(
            [sys.executable, "-m", "py_compile", str(valid_file)],
            "Valid syntax test"
        )
        assert result.success is True
    ...
    def test_working_directory_isolation(self):
        ...
        assert "test_file.txt" in result.output or result.success is False  # ls might not be available on all systems
